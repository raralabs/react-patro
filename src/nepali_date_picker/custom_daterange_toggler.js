    ...
    return (
      <div className="selector-content-wrapper">
        <div className="selector-content">
          {options.map((it) => {
            let is_selected =
              it.dateFrom === selected_date_1 &&
              it.dateTo === selected_date_2 &&
              selected_date_1 &&
              selected_date_2;
            return (
              <Button
                type={is_selected ? "primary" : ""}
                onClick={() => {
                  typeof this.props.onChange === "function" &&
                    this.props.onChange(it.dateFrom, it.dateTo);
                  this.setState({
                    isCustomClicked: false,
                  });
                }}
...
class CustomDateRangeToggler extends Component {
  ...
  render() {
    let label = "Last 7";
    const { selected_date_1, selected_date_2, calendarType, selectorVisible } =
      this.state;
    ...
    if (
      calendarType === "BS" &&
      moment(selected_date_1, "DD-MM-YYYY").isValid()
    ) {
      ...
      rendering_val_1 = `${padDateMonth(bsDateObj.bsDate)}-${padDateMonth(
        bsDateObj.bsMonth
      )}-${bsDateObj.bsYear}`;
    }
    if (
      calendarType === "BS" &&
      moment(selected_date_2, "DD-MM-YYYY").isValid()
    ) {
      ...
      rendering_val_2 = `${padDateMonth(bsDateObj.bsDate)}-${padDateMonth(
        bsDateObj.bsMonth
      )}-${bsDateObj.bsYear}`;
    }
    ...
    let options = [
      {
        ...
        dateFrom: moment().subtract("day", 6).format("DD-MM-YYYY"),
        label: "Last 7",
      },
      {
        dateTo: moment().format("DD-MM-YYYY"),
        dateFrom: moment().subtract("day", 29).format("DD-MM-YYYY"),
        label: "Last 30",
      },
      {
        dateTo: moment().format("DD-MM-YYYY"),
        dateFrom: moment().subtract("day", 59).format("DD-MM-YYYY"),
        label: "Last 60",
      },
      {
        dateTo: moment().format("DD-MM-YYYY"),
        dateFrom: moment().subtract("day", 179).format("DD-MM-YYYY"),
        label: "Last 180",
      },
      {
        dateTo: moment().format("DD-MM-YYYY"),
        dateFrom: moment().subtract("day", 359).format("DD-MM-YYYY"),
        label: "Last 360",
      },
      {
        dateTo: moment().endOf("year").format("DD-MM-YYYY"),
    ...
    return (
      <div className="rl-daterange-toggler">
        ...
        <div className="rl-daterange-toggler-content">
          <div
            className="switch left-switch hand-cursor"
            onClick={() => {
              if (!is_disabled_left) {
                // not disabled right;
                ...
                this.setState({
                  selected_date_2: dt_fr
                    .subtract("day", 1)
                    .format("DD-MM-YYYY"),
                  selected_date_1: dt_fr
                    .subtract("day", diff_days)
                    .format("DD-MM-YYYY"),
                });
              }
            }}
          ...
          <div className="label-body">
            {rendering_val_1 || "DD-MM-YYYY"} ~{" "}
            {rendering_val_2 || "DD-MM-YYYY"}
          </div>
          <div
            className="switch right-switch hand-cursor"
            onClick={() => {
